...
bool PEFile::loadFromFile(const char* filePath) {
    ...
    return readFileData((char*)filePath) &&
        readHeaders() &&
        readBody() &&
        readImportTable();
}
//==============================================================================
bool PEFile::loadFromMemory(const char* memoryAddress) {
    unloadFile();

    peMemory = const_cast<char*>(memoryAddress);
...
//==============================================================================
void PEFile::addImport(const char* dllName, const char** functions, int functionCount) {
    PE_IMPORT_DLL* importDll = &this->newImports;
    PE_IMPORT_FUNCTION* importFunction;
    ...
    importDll->DllName = (char*)dllName;
    importDll->Functions = new PE_IMPORT_FUNCTION();
    importDll->Next = NULL;

    importFunction = importDll->Functions;
    importFunction->FunctionName = (char*)functions[0];
